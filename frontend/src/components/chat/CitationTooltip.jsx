...
function buildShortCitation(paper) {
  ...
  const authors = Array.isArray(paper.authors) ? paper.authors : (paper.authors ? [paper.authors] : []);
  if (authors.length) {
...
export default function CitationTooltip({ havfItem }) {
  const { papers } = usePaperStore();

  if (!havfItem) return null;
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center justify-between gap-2 px-3 py-2 border-b border-tl-b1 rounded-t-lg ${confCls}`}>
        <div className="flex items-center gap-1.5">
          ...
          {shortCite && (
            <span className="text-[10px] font-mono opacity-80">— {shortCite}</span>
          )}
        </div>
