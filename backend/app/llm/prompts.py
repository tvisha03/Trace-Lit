...
System prompts, citation rules, context formatting, and prompt builder
for multi-paper academic Q&A with sentence-level attribution.
"""
...
def sanitize_user_input(query: str) -> str:
    ...
    for pattern in _DANGEROUS_PATTERNS:
        if pattern.lower() in sanitized.lower():
            ...
            logger.warning(f"Prompt injection pattern filtered: '{pattern}'")
...
def format_context_block(paragraphs: List[Dict]) -> str:
    ...
    Each paragraph gets a clear [P#] identifier with paper/section metadata
...
def trim_context_to_budget(
    ...
    # Reserve budget for system prompt, history, and response
    ...
    if len(selected) < len(paragraphs):
        logger.debug(
            f"Trimmed context from {len(paragraphs)} to {len(selected)} "
            f"paragraphs for {provider} (budget: {context_budget} tokens)"
        )
...
def assemble_prompt(
    ...
    logger.debug(
        f"Prompt assembled: system={estimate_tokens(system)} tokens, "
        f"user={estimate_tokens(user_prompt)} tokens, "
        f"provider={provider}, paragraphs={len(trimmed)}"
    )
...
def validate_citations(
    ...
    # Split response into sentences and check for uncited factual claims
...
def _split_response_sentences(text: str) -> List[str]:
    ...
    Handles academic abbreviations and citation patterns.
    """
...
def _is_factual_claim(sentence: str) -> bool:
    ...
    Transitional phrases and meta-commentary don't need citations.
    """
